...
**ขึ้น `timeout` เป็นครั้งคราวทั้งที่บัตรเสียบอยู่**
มักเกิดจากบัตรเสียบไม่สุดหรือขยับระหว่างอ่าน **ถอดบัตรออกแล้วเสียบใหม่ให้สุด**
ตัวเฝ้ารอจะอ่านใหม่เองเมื่อตรวจพบว่าเสียบบัตรอีกครั้ง

จากการทดสอบจริงแบบเปิดโปรเซสค้างแล้วเสียบ-ถอดบัตรต่อเนื่อง อ่านสำเร็จ 9 ครั้งติด
และหลังเจอ timeout หนึ่งครั้ง ระบบยังตรวจจับการถอด/เสียบบัตรได้ตามปกติ
ไม่จำเป็นต้องรีสตาร์ตหรือถอดสาย USB

**ขึ้น `timeout` ทุกครั้งไม่ว่าจะทำอะไร และ ATR ยังอ่านได้**
กรณีนี้คือ slot ของ PC/SC daemon ค้างจริง มักเกิดเมื่อมีโปรเซสถูกปิดกลางคัน
ระหว่างที่กำลังเชื่อมต่อบัตร วิธีแก้ตามลำดับ:
...
ตรวจว่าใช่อาการนี้ไหม: `system_profiler SPSmartCardsDataType` ยังเห็น ATR
แต่โปรแกรมต่อบัตรไม่ได้ และชื่อเครื่องอ่านมีเลขต่อท้าย (เช่น `... Reader 01`)
ซึ่งเป็นสัญญาณว่ามี slot เก่าค้างอยู่
